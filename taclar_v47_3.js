...
let firstFirebaseRender=true;
function refreshLiveDataWithoutReset(){
  ...
  if(page==='booking'){renderBooking();return}
  if(page==='validation'){renderValidation();return}
  if(page==='publish'){if($('activeDriversBox'))renderActiveDrivers();return}
  // Sur les pages contenant des formulaires chauffeur, ne jamais reconstruire toute
  // l'interface à chaque synchronisation Firestore. Les actions utilisateur
  // appellent déjà renderPage() lorsqu'un changement d'écran est nécessaire.
}
function initFirebase(){
  const cfg=window.TACLAR_FIREBASE_CONFIG;
  if(!cfg||String(cfg.apiKey||'').includes('COLLER_')){showLoad('Firebase non configuré');return}
  firebase.initializeApp(cfg);
  db=firebase.firestore();
  showLoad('Connexion Firebase...');
  db.collection(collectionName).onSnapshot(snap=>{
    docs=snap.docs.map(d=>({id:d.id,...d.data()}));
    ready=true;
    showLoad('Synchronisé');
    setTimeout(()=>showLoad(''),900);
    if(firstFirebaseRender){
      firstFirebaseRender=false;
      renderPage();
    }else{
      refreshLiveDataWithoutReset();
    }
