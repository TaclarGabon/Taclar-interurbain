...
async function loadClientDataOnce(){
  const snap=await db.collection(collectionName).get();
  docs=snap.docs.map(d=>({id:d.id,...d.data()}));
  ready=true;
}
async function refreshClientData(){
  await loadClientDataOnce();
...
function initFirebase(){
  ...
  if(document.body.dataset.page==='client'){
    loadClientDataOnce().then(()=>{
      showLoad('Synchronisé');
      setTimeout(()=>showLoad(''),900);
      renderPage();
    }).catch(err=>showLoad('Erreur Firebase: '+err.message));
    return;
  }
...
async function declareClientPayment(id){await updateDoc(id,{status:'payment_declared',paymentDeclaredAt:Date.now()});await refreshClientData()}
