...
let driverSessionMemory=null,driverLocalAction=false,driverReconnectNotice='';
...
function getDriverSession(){return driverSessionMemory}
function setDriverSession(id,pin,status){driverSessionMemory={id,pin,status}}
function clearDriverSession(){driverSessionMemory=null}
...
function renderNewDriver(){
  ...
  if(current){
    if(session.status&&current.status!==session.status&&!driverLocalAction){
      driverReconnectNotice=`<div class="message-banner"><strong>Nouvelle mise à jour TACLAR : ${statusText(current.status)}</strong><br><span>Notification simulée WhatsApp / e-mail envoyée. Reconnecte-toi avec ton numéro de dossier et ton code PIN pour consulter ton dossier.</span></div>`;
      clearDriverSession();
    }else{
      if(driverSessionMemory)driverSessionMemory.status=current.status;
      driverLocalAction=false;
      renderDriverDashboard(current);return
    }
  }
  if(session&&!current)clearDriverSession();
  const reconnectNotice=driverReconnectNotice; driverReconnectNotice='';
  setShell('new',`${reconnectNotice}<div class="grid two"><div class="card"><h2>Nouveau chauffeur - rejoindre TACLAR</h2><div class="driver-info-tip"><div class="tip-icon">💡</div><div><strong>À lire avant de commencer</strong><p>Renseigne tes informations de chauffeur : nom, téléphone, véhicule, plaque, axe souhaité, nombre de places et prix du transport.</p><p>Envoie ensuite tes documents pour vérification. Après validation par TACLAR, tu recevras une notification WhatsApp / e-mail pour te reconnecter avec ton numéro de dossier et ton code PIN.</p><p>Après paiement et confirmation des frais d’enregistrement, ton compte sera activé et tu pourras publier tes trajets.</p></div></div><div class="field-grid"><div><label>Nom et prénom chauffeur</label><input id="signupName" placeholder="Ex : Serge Ndong"></div><div><label>Téléphone chauffeur</label><input id="signupPhone" placeholder="Ex : +241 77 45 90 12"></div><div><label>Véhicule</label><input id="signupVehicle" placeholder="Ex : Toyota Noah"></div><div><label>Plaque</label><input id="signupPlate" placeholder="Ex : GA-421-LB"></div><div><label>Axe souhaité</label><select id="signupAxis"></select></div><div><label>Prix transport / place</label><input id="signupPrice" disabled></div><div><label>Nombre de places</label><input id="signupSeats" type="number" min="1" step="1" placeholder="Ex : 6"></div><div><label>Frais d’enregistrement calculés</label><input id="signupDeposit" value="0 FCFA" disabled></div><div><label>Permis de conduire</label><input id="signupLicense" type="file"></div><div><label>Pièce d'identité</label><input id="signupIdCard" type="file"></div></div><div class="notice warning"><strong>Étape 1 :</strong> soumettre le dossier chauffeur. Les fichiers sont simulés par leur nom dans cette démo.</div><div id="signupResult" class="notice success hidden"></div><div class="actions"><button id="submitDriver">Soumettre le dossier chauffeur</button><button class="secondary" id="clearDriver">Vider le formulaire</button></div></div><div class="card"><h2>Déjà inscrit ?</h2><p>Entre ton numéro de dossier et ton code PIN pour consulter uniquement ton propre dossier. Après chaque mise à jour TACLAR, reconnecte-toi ici.</p><div><label>Numéro de dossier</label><input id="loginDriverCode" placeholder="Ex : CH-12345"></div><div style="margin-top:12px"><label>Code PIN</label><input id="loginDriverPin" inputmode="numeric" maxlength="4" placeholder="4 chiffres"></div><div class="actions"><button id="loginDriver">Ouvrir mon dossier</button></div><div id="loginMsg" class="notice danger hidden"></div></div></div>`);
...
async function submitDriver(){
  ...
  await addDoc({type:'driverApplication',driverCode,pin,name,phone,vehicle,plate,axis,seats,price:axisPrices[axis]||0,deposit:seats*taclarFee,licenseName:$('signupLicense').files[0]?.name||'Non joint',idCardName:$('signupIdCard').files[0]?.name||'Non joint',status:'submitted',docsValidated:false,depositPaid:false,depositValidated:false,active:false,notifications:[{kind:'submitted',title:'Dossier envoyé',text:'Votre dossier a été envoyé à TACLAR.',at:Date.now(),read:false}]});
  const result=$('signupResult');
  if(result){result.classList.remove('hidden');result.innerHTML=`<strong>Dossier envoyé à TACLAR.</strong><br>Numéro de dossier : <b>${driverCode}</b><br>Code PIN : <b>${pin}</b><br>Conserve ces informations. Pour suivre ton dossier, utilise la zone <b>Déjà inscrit ?</b>.`;}
...
function loginDriver(){
  ...
  setDriverSession(app.id,pin,app.status);
  renderPage();
}
function renderDriverDashboard(a){
  ...
  let banner='';
  if(a.docsValidated&&!a.depositPaid){message=`Tes documents sont approuvés. Paie maintenant les frais d’enregistrement de ${money(a.deposit)}.`;banner='Nouveau message : documents approuvés. Connecte-toi pour payer les frais d’enregistrement.';}
  else if(a.depositPaid&&!a.depositValidated){message='Tes frais d’enregistrement sont déclarés payés. TACLAR vérifie maintenant leur réception.';banner='Nouveau message : paiement déclaré. TACLAR vérifie la réception des frais.';}
  else if(a.depositValidated&&!a.active){message='Tes frais d’enregistrement sont reçus. TACLAR doit encore autoriser la publication.';banner='Nouveau message : frais reçus. Autorisation de publication en attente.';}
  else if(a.active){message='Ton dossier est complet. Tu peux maintenant publier un trajet.';banner='Nouveau message : autorisation de publier votre trajet.';}
  const publishBlock=a.active?`<div class="card driver-publish-card"><h2>Publier mon trajet</h2><p class="muted">Tes informations chauffeur sont déjà enregistrées. Complète seulement la date, les heures et le point d’embarquement.</p><div class="field-grid"><div><label>Date de départ</label><input id="driverPubDay" type="date"></div><div><label>Heure d’enregistrement</label><select id="driverPubCheckin"></select></div><div><label>Heure de départ</label><select id="driverPubTime"></select></div><div><label>Point d’embarquement</label><input id="driverPubBoarding" placeholder="Ex : Gare routière"></div><div><label>Places réellement disponibles</label><input id="driverPubSeats" type="number" min="1" value="${a.seats||1}"></div><div><label>Prix / place</label><input value="${money(a.price)}" disabled></div></div><div class="actions center-actions"><button class="green" id="driverPubBtn">Publier le trajet</button></div><div id="driverPubMsg" class="notice hidden"></div><div class="notice"><strong>Info :</strong> après publication, ton trajet apparaît dans Booking / Disponibilités et côté Client.</div></div>`:'';
  setShell('new',`${banner?`<div class="message-banner"><strong>${banner}</strong><br><span>Notification simulée : WhatsApp / e-mail envoyé. Le chauffeur doit se reconnecter avec son dossier et son code PIN pour voir cette mise à jour.</span></div>`:''}<div class="card driver-space"><div class="item-top"><div><h2>Suivi de mon dossier chauffeur</h2><p class="muted">Bienvenue, <strong>${a.name}</strong>.</p></div><span class="badge ${a.active?'ok':'warn'}">${statusText(a.status)}</span></div><div class="credentials"><div><small>Numéro de dossier</small><strong>${a.driverCode||'Ancien dossier'}</strong></div><div><small>Code PIN personnel</small><strong>${a.pin||'Non défini'}</strong></div></div><div class="notice warning"><strong>À conserver :</strong> note ton numéro de dossier et ton code PIN pour te reconnecter depuis un autre téléphone.</div><div class="timeline">${steps.map(s=>`<div class="${s.done?'done':''}"><span>${s.done?'✓':'○'}</span><strong>${s.label}</strong></div>`).join('')}</div><div class="notice ${a.active?'success':''}">${message}</div><div class="facts"><div class="fact"><small>Nom</small><strong>${a.name}</strong></div><div class="fact"><small>Téléphone</small><strong>${a.phone}</strong></div><div class="fact"><small>Véhicule</small><strong>${a.vehicle}</strong></div><div class="fact"><small>Plaque</small><strong>${a.plate}</strong></div><div class="fact"><small>Axe souhaité</small><strong>${a.axis}</strong></div><div class="fact"><small>Places déclarées</small><strong>${a.seats}</strong></div><div class="fact"><small>Frais d’enregistrement</small><strong>${money(a.deposit)}</strong></div><div class="fact"><small>Statut</small><strong>${statusText(a.status)}</strong></div></div>${a.docsValidated&&!a.depositPaid?`<div class="actions"><button class="orange" onclick="driverPayDeposit('${a.id}')">Déclarer les frais payés</button></div>`:''}</div>${publishBlock}<div class="driver-logout-row"><button class="secondary" onclick="logoutDriver()">Se déconnecter</button></div>`);
  if(a.active){fillTimeSelect($('driverPubCheckin'),'-- Heure enregistrement --');fillTimeSelect($('driverPubTime'),'-- Heure départ --');$('driverPubBtn').onclick=()=>driverPublishOffer(a.id)}
}

async function driverPublishOffer(appId){
  const a=apps().find(x=>x.id===appId);
  if(!a||!a.active){alert('Ton dossier n’est pas encore autorisé à publier.');return}
  const day=$('driverPubDay').value,checkinTime=$('driverPubCheckin').value,time=$('driverPubTime').value,boarding=$('driverPubBoarding').value.trim(),seats=Number($('driverPubSeats').value||0);
  if(!day||!checkinTime||!time||!boarding||seats<1){alert('Complète date, heures, point d’embarquement et places.');return}
  await addDoc({type:'offer',driverAppId:a.id,driver:a.name,phone:a.phone,vehicle:a.vehicle,plate:a.plate,axis:a.axis,seats,booked:0,price:a.price,day,checkinTime,time,boarding,status:'Disponible',source:'driver_space'});
  const msg=$('driverPubMsg');
  if(msg){msg.className='notice success';msg.textContent='Trajet publié. Il apparaît maintenant dans Booking / Disponibilités et côté Client.'}
  $('driverPubDay').value='';$('driverPubCheckin').value='';$('driverPubTime').value='';$('driverPubBoarding').value='';
}

async function driverPayDeposit(id){if(!confirm('Confirmer que les frais d’enregistrement ont été envoyés à TACLAR ?'))return;driverLocalAction=true;await updateDoc(id,{status:'deposit_paid',depositPaid:true,depositPaidAt:Date.now()})}
...
function renderValidation(){
  ...
  setShell('validation',`<div class="card"><h2>Validation TACLAR / Admin</h2><p>Vue interne TACLAR : validation des chauffeurs, paiements clients déclarés, frais d’enregistrement et vue d’ensemble.</p><div class="kpi admin-kpi"><div><b>${totalDispos}</b><span>disponibilités</span></div><div><b>${totalRequests}</b><span>demandes</span></div><div><b>${paidPlaces}</b><span>places payées</span></div><div><b>${money(taclarTotal)}</b><span>frais TACLAR</span></div></div><div class="notice warning"><strong>Important :</strong> les confirmations de paiement client se traitent ici. Booking affiche le statut, mais TACLAR/Admin confirme la réception.</div><h3 style="margin-top:16px">Paiements clients à confirmer</h3><div id="clientPaymentsList" class="compact-list" style="margin-top:14px"></div><h3 style="margin-top:16px">Dossiers chauffeurs</h3><div id="applicationsList" class="compact-list" style="margin-top:14px"></div></div>`);
  renderClientPaymentsList();
  renderApplicationsList();
}
function renderClientPaymentsList(){
  const box=$('clientPaymentsList');
  if(!box)return;
  const list=requests().filter(r=>r.status==='payment_pending').sort((a,b)=>(b.updatedAt||b.paymentDeclaredAt||b.createdAt||0)-(a.updatedAt||a.paymentDeclaredAt||a.createdAt||0));
  if(!list.length){box.innerHTML='<div class="notice">Aucun paiement client déclaré en attente de confirmation.</div>';return}
  box.innerHTML=list.map(r=>{const o=offers().find(x=>x.id===r.offerId)||{};return `<details class="compact-item admin-payment" open><summary><span><strong>${o.driver||'Chauffeur'} | ${shortAxisName(o.axis)} | Client : ${r.groupLeader||r.clientName} | ${r.seats||1} place(s)</strong><small>Paiement déclaré - confirmation TACLAR en attente</small></span><span class="badge warn">À confirmer</span></summary><div class="compact-body"><div class="facts"><div class="fact"><small>Client</small><strong>${r.groupLeader||r.clientName}</strong></div><div class="fact"><small>Téléphone client</small><strong>${r.clientPhone||'-'}</strong></div><div class="fact"><small>Chauffeur</small><strong>${o.driver||'-'}</strong></div><div class="fact"><small>Axe</small><strong>${o.axis||'-'}</strong></div><div class="fact"><small>Places</small><strong>${r.seats||1}</strong></div><div class="fact"><small>Frais TACLAR</small><strong>${money(Number(r.seats||1)*taclarFee)}</strong></div><div class="fact"><small>Jour</small><strong>${formatDateFr(o.day)}</strong></div><div class="fact"><small>Embarquement</small><strong>${o.boarding||'-'}</strong></div></div><div class="notice warning">Le client a déclaré le paiement Airtel Money. Confirme seulement après réception réelle des frais TACLAR.</div><div class="actions"><button class="green" onclick="confirmClientPayment('${r.id}')">Confirmer paiement reçu</button><button class="red" onclick="deleteDocHard('${r.id}')">Supprimer erreur</button></div></div></details>`}).join('')
}
async function confirmClientPayment(requestId){
  await updateDoc(requestId,{status:'paid',paidAt:Date.now(),paymentConfirmedAt:Date.now()});
}
...
async function validateDriverDocuments(id){await updateDoc(id,{status:'docs_validated',docsValidated:true,docsValidatedAt:Date.now(),lastNotification:'Documents approuvés - reconnectez-vous pour payer les frais'})}
async function validateDriverDeposit(id){await updateDoc(id,{status:'deposit_validated',depositValidated:true,depositValidatedAt:Date.now(),lastNotification:'Frais reçus - autorisation de publier en attente'})}
async function authorizeDriver(id){await updateDoc(id,{status:'active',active:true,authorizedAt:Date.now(),lastNotification:'Autorisation de publier votre trajet'})}
...
async function publishOffer(){const a=activeApps().find(x=>x.id===$('publishDriver').value);if(!a){alert('Choisissez un chauffeur validé.');return}const day=$('publishDay').value,checkinTime=$('publishCheckin').value,time=$('publishTime').value,boarding=$('publishBoarding').value.trim(),seats=Number($('publishSeats').value||0);if(!day||!checkinTime||!time||!boarding||seats<1){alert('Complétez date, heures, embarquement et places.');return}await addDoc({type:'offer',driverAppId:a.id,driver:a.name,phone:a.phone,vehicle:a.vehicle,plate:a.plate,axis:a.axis,seats,booked:0,price:a.price,day,checkinTime,time,boarding,status:'Disponible',source:'admin_publish'});$('publishMsg').className='notice success';$('publishMsg').textContent='Trajet publié. Il apparaît maintenant côté client et booking.';$('publishDay').value='';$('publishCheckin').value='';$('publishTime').value='';$('publishBoarding').value=''}
...
function renderBookingRequest(r,o){
  ...
  if(r.status==='payment_pending') actions.push(`<button class="secondary" disabled>À confirmer dans Validation TACLAR</button>`);
