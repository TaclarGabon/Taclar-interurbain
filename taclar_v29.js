...
const paymentHoldMs=paymentHoldMinutes*60*1000;
...
function liveReqsForOffer(id){return reqsForOffer(id).filter(r=>!['refused','deleted','expired','payment_not_received','cancelled'].includes(r.status))}
...
function paymentDeadline(r){return Number(r.paymentDeadlineAt||0)}
function isPaymentExpired(r){return r.status==='confirmed'&&paymentDeadline(r)>0&&Date.now()>paymentDeadline(r)}
function remainingPaymentMs(r){return Math.max(0,paymentDeadline(r)-Date.now())}
function countdownText(r){const left=remainingPaymentMs(r);if(!left)return 'expiré';const m=Math.floor(left/60000);const sec=Math.floor((left%60000)/1000);return `${String(m).padStart(2,'0')}:${String(sec).padStart(2,'0')}`}
function paymentTimerHtml(r){if(!paymentDeadline(r))return '';return `<div class="notice warning payment-timer"><strong>Délai de paiement :</strong> ${countdownText(r)} avant libération automatique des places.</div>`}
async function expireRequest(id,reason='Délai dépassé'){
  const r=requests().find(x=>x.id===id);if(!r||!['confirmed'].includes(r.status))return;
  await updateDoc(id,{status:'expired',expiredAt:Date.now(),expiredReason:reason,systemNote:'Places remises disponibles automatiquement'});
}
async function expireOverdueRequests(){
  if(!db||!ready)return;
  const overdue=requests().filter(isPaymentExpired);
  for(const r of overdue){try{await expireRequest(r.id,'Délai de paiement dépassé')}catch(e){console.warn('Expiration impossible',e)}}
}
async function driverConfirmRequest(id){
  const r=requests().find(x=>x.id===id);if(!r)return;
  const deadline=Date.now()+paymentHoldMs;
  await updateDoc(id,{status:'confirmed',confirmedAt:Date.now(),paymentDeadlineAt:deadline,paymentHoldMinutes});
}
async function markClientPaymentNotReceived(id){
  if(!confirm('Confirmer que le paiement client n’a pas été reçu ? Les places seront remises disponibles.'))return;
  await updateDoc(id,{status:'payment_not_received',paymentNotReceivedAt:Date.now(),systemNote:'Paiement non reçu par TACLAR - places remises disponibles'});
}
function refreshCountdowns(){
  document.querySelectorAll('[data-countdown-request]').forEach(el=>{const r=requests().find(x=>x.id===el.dataset.countdownRequest);if(r)el.textContent=countdownText(r)});
}
setInterval(()=>{refreshCountdowns();expireOverdueRequests()},15000);
function statusText(s){return {submitted:'Documents en cours de traitement',docs_validated:'Documents approuvés - caution à déposer',deposit_paid:'Caution déposée - vérification TACLAR',deposit_validated:'Caution reçue - autorisation en attente',active:'Autorisé à publier',pending:'Demande envoyée',confirmed:'Confirmée chauffeur - paiement à faire',payment_declared:'Paiement déclaré - validation TACLAR',paid:'Commission TACLAR payée',payment_not_received:'Paiement non reçu',expired:'Réservation expirée',refused:'Refusée',deleted:'Supprimée',cancelled:'Annulée'}[s]||s||'-'}
...
async function confirmClientPayment(id){await updateDoc(id,{status:'paid',paidAt:Date.now(),paymentValidatedAt:Date.now()})}
...
function exportTaclarHistory(){
  const rows=[['Date création','Référence','Statut','Client','Téléphone','Axe','Chauffeur','Places','Montant TACLAR','Confirmé le','Paiement déclaré le','Validé le','Expiration / refus']];
  requests().sort((a,b)=>(b.createdAt||0)-(a.createdAt||0)).forEach(r=>{
    const o=offers().find(x=>x.id===r.offerId)||{};
    const d=t=>t?new Date(t).toLocaleString('fr-FR'):'';
    rows.push([d(r.createdAt),r.requestCode||'',statusText(r.status),r.groupLeader||r.clientName||'',r.clientPhone||'',o.axis||'',o.driver||'',r.seats||1,Number(r.seats||1)*taclarFee,d(r.confirmedAt),d(r.paymentDeclaredAt),d(r.paymentValidatedAt||r.paidAt),d(r.expiredAt||r.paymentNotReceivedAt||r.refusedAt)]);
  });
  const csv=rows.map(row=>row.map(csvEscape).join(';')).join('\n');
  const blob=new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8'});
  const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='historique_taclar_interurbain.csv';a.click();URL.revokeObjectURL(a.href);
}
async function declareClientPayment(id){const r=requests().find(x=>x.id===id);if(!r)return;if(isPaymentExpired(r)){await expireRequest(id,'Délai de paiement dépassé');alert('Le délai de paiement est dépassé. Les places sont remises disponibles.');return}await updateDoc(id,{status:'payment_declared',paymentDeclaredAt:Date.now()})}
